...
Inputs (read only, all pre-existing BoltzTraP2 outputs):
  boltztrap2/Rb2Cu2SnS4.trace      - btp2 integrate mu scan, 300-900 K
  boltztrap2/Rb2Cu2SnS4.condtens   - full tensors (cross-check of the scalar S)
  boltztrap2/Rb2Cu2SnS4.dope.trace - fixed-density run (consistency check)
  boltztrap2/Rb2Cu2SnS4.bt2        - E_F
  structures/Rb2Cu2SnS4.relaxed.cif- cell volume for density conversion
...
GAP_EV = 0.7811                      # QE-PBE sampled gap; VBM at E - E_F = 0
...
def main():
    data, *_ = serialization.load_calculation(str(BT / "Rb2Cu2SnS4.bt2"))
    fermi_ry = data.fermi * 2.0          # Ha -> Ry
    nelect = data.nelect                 # electrons in BoltzTraP2's band window
    volume_cm3 = Structure.from_file(
        CAND / "structures" / "Rb2Cu2SnS4.relaxed.cif").volume * 1e-24

    trace = np.loadtxt(BT / "Rb2Cu2SnS4.trace", comments="#")
    cond = np.loadtxt(BT / "Rb2Cu2SnS4.condtens", comments="#")
    dope = np.loadtxt(BT / "Rb2Cu2SnS4.dope.trace", comments="#")
    ...
    target_n = 1e20 * volume_cm3         # p-type (electrons removed)
    ...
    ax.set_title(r"Rb$_2$Cu$_2$SnS$_4$ Seebeck coefficient, centered on $E_\mathrm{F}$"
