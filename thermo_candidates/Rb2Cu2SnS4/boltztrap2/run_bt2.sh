...
qe_source="qe/tmp/final/Rb2Cu2SnS4.save"
bt2_file="boltztrap2/Rb2Cu2SnS4.bt2"
log_dir="logs"
workers="${BT2_NP:-4}"
...
if [ ! -f "$bt2_file" ]; then
  "${btp2_cmd[@]}" -n "$workers" -v interpolate -m 5 -o "$bt2_file" "$qe_source" \
    2>&1 | tee "$log_dir/Rb2Cu2SnS4.bt2.interpolate.log"
fi

"${btp2_cmd[@]}" -v integrate "$bt2_file" 300:1000:100 \
  2>&1 | tee "$log_dir/Rb2Cu2SnS4.bt2.integrate.log"
"${btp2_cmd[@]}" -v dope "$bt2_file" 300:1000:100 "$doping_levels" \
  2>&1 | tee "$log_dir/Rb2Cu2SnS4.bt2.dope.log"
