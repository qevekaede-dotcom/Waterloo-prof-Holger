#!/usr/bin/env python3
"""Plot the Seebeck coefficient vs chemical potential, centered on E_F.

Inputs (read only, all pre-existing BoltzTraP2 outputs):
  boltztrap2/SrCu2SnS4.trace      - btp2 integrate mu scan, 300-900 K
  boltztrap2/SrCu2SnS4.condtens   - full tensors (cross-check of the scalar S)
  boltztrap2/SrCu2SnS4.dope.trace - fixed-density run (consistency check)
  boltztrap2/SrCu2SnS4.bt2        - E_F
  structures/SrCu2SnS4.relaxed.cif- cell volume for density conversion
...
def main():
    data, *_ = serialization.load_calculation(str(BT / "SrCu2SnS4.bt2"))
    fermi_ry = data.fermi * 2.0          # Ha -> Ry
    nelect = data.nelect                 # 124 in BoltzTraP2's band window
    volume_cm3 = Structure.from_file(
        CAND / "structures" / "SrCu2SnS4.relaxed.cif").volume * 1e-24

    trace = np.loadtxt(BT / "SrCu2SnS4.trace", comments="#")
    cond = np.loadtxt(BT / "SrCu2SnS4.condtens", comments="#")
    dope = np.loadtxt(BT / "SrCu2SnS4.dope.trace", comments="#")
    ...
    # The trace N column is relative to the neutral cell (positive = electrons
    # removed, i.e. p-type), so it must cross ~0 exactly at mu = E_F.
    ...
    # --- check 3: consistency with the doped run (300 K, p-type 1e20) ------
    target_n = 1e20 * volume_cm3         # +0.0547 e/uc, p-type
    d300 = dope[dope[:, 1] == 300]
    row = d300[np.argmin(np.abs(d300[:, 2] - target_n))]
    s_dope = row[4] * 1e6
    s_scan = np.interp(row[0], row300[:, 0], row300[:, 4]) * 1e6
    print(f"check 3: 300 K p-type 1e20 cm^-3 -> dope S = {s_dope:.1f} uV/K, "
    ...
    ax.set_title(r"SrCu$_2$SnS$_4$ Seebeck coefficient, centered on $E_\mathrm{F}$"
    ...
    # peak values inside the +/-1 eV window at 300 K, for the summary
    m = trace[:, 1] == 300
    x, y = mu_ev[m], s_uv[m]
    inwin = np.abs(x) <= win
